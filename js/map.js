...
class MAP {
  ...
  textChairTable ( idZona , sentido ) {
    try {
      ...
      SVG.select( `#${idZona} g[typeGroup="table"]` ).members.forEach( ( table ) => {
        length = table.node.children.length

        if ( sentido === 'horario' ) {
          cont = 1

          for (let index = ( length/2 )-1; index >= 1; index--) {
            id = table.node.childNodes[index].id
            textContent = table.node.childNodes[index+( length/2 )].textContent
          ...
          for (let index = length-1; index >= ( length/2 )+1; index--) {
            id = table.node.childNodes[index].id
            textContent = table.node.childNodes[index].textContent
